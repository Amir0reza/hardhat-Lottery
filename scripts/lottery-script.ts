...
import { PURCHASE_RATIO, BET_PRICE, BET_FEE } from "../deploy/01-deploy-Lottery"
import * as readline from "readline"

let lottery: Lottery
let paymentToken: LotteryTokenClassic
...
async function checkState() {
    const state = await lottery.getBetsOpen()
...
async function buyTokens(index: string, amount: string) {
    const tx = await lottery.connect(accounts[Number(index)]).purchaseTokens({
...
async function displayTokenBalance(index: string) {
    const balanceBN = await paymentToken.balanceOf(
        accounts[Number(index)].address
    )
    const balance = ethers.utils.formatEther(balanceBN)
    console.log(
...
async function bet(index: string, amount: string) {
    ...
    await allowTx.wait()
    const tx = await lottery.connect(accounts[Number(index)]).betMany(amount)
    const receipt = await tx.wait()
...
async function displayPrize(index: string): Promise<string> {
    const prizeBN = await lottery.getPrize(accounts[Number(index)].address)
...
async function displayOwnerPool() {
    const balanceBN = await lottery.getOwnerPool()
    const balance = ethers.utils.formatEther(balanceBN)
    console.log(`The owner pool has (${balance}) Tokens \n`)
}

async function withdrawTokens(amount: string) {
    const tx = await lottery.ownerWithdraw(ethers.utils.parseEther(amount))
    const receipt = await tx.wait()
